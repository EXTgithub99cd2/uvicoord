...
This is the core CLI with cross-platform commands.
Platform-specific service management is in separate packages:
- uvicoord-windows: Windows Task Scheduler integration
- uvicoord-nixos: NixOS/systemd integration
"""
...
def service_stop():
    ...
    if not check_service():
        rprint("[yellow]Service is not running.[/yellow]")
        return
    
    rprint("[yellow]Service must be stopped manually (Ctrl+C or kill process).[/yellow]")
    rprint("Platform-specific packages provide better stop functionality:")
    rprint("  - Windows: [cyan]pip install uvicoord-windows[/cyan]")
    rprint("  - NixOS: [cyan]pip install uvicoord-nixos[/cyan] or use systemctl")
...
def list_apps():
    ...
    for name, app_info in apps.items():
        ports_str = ", ".join(map(str, app_info["available_ports"]))
        if len(ports_str) > 30:
            ports_str = ports_str[:27] + "..."
        table.add_row(
            name,
            app_info["path"],
            app_info["port_strategy"],
...
def _get_venv_activation(app_path: Path) -> tuple[Optional[str], str]:
    """
    Get venv activation command and shell based on platform.
    
    Returns:
        Tuple of (activation_command, shell_type)
        activation_command is None if no venv found
    """
    if sys.platform == "win32":
        # Windows: PowerShell
        venv_activate = app_path / ".venv" / "Scripts" / "Activate.ps1"
        if venv_activate.exists():
            return f'& "{venv_activate}"', "powershell"
        return None, "powershell"
    else:
        # Unix: bash
        venv_activate = app_path / ".venv" / "bin" / "activate"
        if venv_activate.exists():
            return f"source {venv_activate}", "bash"
        return None, "bash"
...
def run_app(
    ...
    # Build the full command with port
    if "--port" in command:
    ...
    # Get venv activation
    venv_cmd, shell_type = _get_venv_activation(app_path)
    
    if venv_cmd:
        full_command = f"{venv_cmd}; {command}"
    else:
